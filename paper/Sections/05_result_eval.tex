...
We evaluate VEXMLM using intrinsic tokenization quality metrics and extrinsic downstream task performance on Amharic and Tigrinya. NER and SA accuracy and F1 are given as fractions to four decimals, QA Exact Match and F1 and the ablation accuracies as percentages to two decimals, and tokenizer metrics to four decimals. VEXMLM scores are means $\pm$ sample standard deviations over five seeds, and differences are computed from unrounded means, so they may differ by one unit in the last decimal from differences of the displayed values.
...
\bottomrule
\end{tabular}
\caption{Tokenizer metrics on held-out Amharic and Tigrinya text: fertility (tokens per word, lower is better), compression (characters per token, higher is better) and the share of OOV words (development-set words absent from the training lines) that the tokenizer reproduces exactly on decoding. Bold indicates the best fertility and compression. All three tokenizers are measured on the same development sets; the XLM-R and VEXMLM values are recorded in \texttt{results/spmerge\_tokenizer\_metrics.json} and the Glot500 values in \texttt{results/provenance/tokenizer\_metrics\_2026-08-15.json}, which agree on XLM-R.}
\label{tab:tokenizer}
\end{table}
