...
Code, configurations and result files are available at \url{https://github.com/hailaykidu/VEXMLM} (release tag \texttt{v1.1-correction}); the released model is \url{https://huggingface.co/Hailay/VEXMLM}. Every result number, table and figure in this paper is generated from the stored result files by \texttt{scripts/make\_paper\_artifacts.py}, which reads the result files written by \texttt{evaluation/export\_spm\_results.py} (\texttt{results/downstream\_task\_metrics.csv}) and \texttt{evaluation/aggregate\_ablation.py} (\texttt{results/table5\_ablation.csv}); \texttt{REPRODUCE.md} maps each table and figure to its producing script, configuration and command, and records where \texttt{scripts/reproduce\_paper.sh} differs from the commands that produced the released artifacts.
